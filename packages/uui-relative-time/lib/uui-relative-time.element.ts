...
function getUnitFactor(el: UUIRelativeTimeElement): number {
...
const dateObserver = new (class {
  elements: Set<UUIRelativeTimeElement> = new Set();
  time = Infinity;

  observe(element: UUIRelativeTimeElement) {
  ...
  unobserve(element: UUIRelativeTimeElement) {
    if (!this.elements.has(element)) return;
    this.elements.delete(element);
  }

  timer: ReturnType<typeof setTimeout> = -1 as unknown as ReturnType<
    typeof setTimeout
  >;
  update() {
    clearTimeout(this.timer);
...
export class UUIRelativeTimeElement
  extends HTMLElement
  implements Intl.DateTimeFormatOptions
{
  #customTitle = false;
  ...
  #resolveFormat(duration: Duration): Format {
  ...
  get year() {
    ...
    if (
      ...
      new Date().getUTCFullYear() !== this.date?.getUTCFullYear()
    ) {
      return 'numeric';
    }
  ...
  attributeChangedCallback(
    ...
    if (attrName === 'title') {
      this.#customTitle =
        newValue !== null &&
        (this.date && this.#getFormattedTitle(this.date)) !== newValue;
    }
  ...
  update() {
    ...
    if (newText !== oldText || newTitle !== oldTitle) {
      this.dispatchEvent(
        new UUIRelativeTimeUpdatedEvent(oldText, newText, oldTitle, newTitle),
      );
    }
