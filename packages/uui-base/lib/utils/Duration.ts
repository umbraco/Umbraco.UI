...
export type DurationFormatOptions = Partial<
  Omit<DurationFormatResolvedOptions, 'locale'>
>;
...
export default class UUIDurationFormat {
  ...
  constructor(locale: string, options: DurationFormatOptions = {}) {
    ...
    this.#options = {
      ...
      milliseconds,
      millisecondsDisplay:
        options.millisecondsDisplay === 'always' ? 'always' : 'auto',
    };
  }
...
export class Duration {
  ...
  static from(durationLike: unknown): Duration {
    if (typeof durationLike === 'string') {
      ...
      const {
        ...
        seconds,
        milliseconds,
      } = durationLike as Record<string, number>;
      return new Duration(
        years,
        months,
        weeks,
  ...
  toLocaleString(locale: string, opts: DurationFormatOptions) {
    return new UUIDurationFormat(locale, opts).format(this);
  }
}
...
export function roundToSingleUnit(
  duration: Duration,
  { relativeTo = Date.now() }: Partial<RoundingOpts> = {},
): Duration {
  ...
  if (days >= 27 || years + months + days) {
    ...
    const daysDiff =
      Math.abs(Math.round((Number(newDate) - Number(relativeTo)) / 86400000)) +
      monthDateCorrection;
