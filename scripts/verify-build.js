...
// stay reachable as a bare import so all registrations run on `import '@umbraco-ui/uui'`.
assert(
...
// ---------------------------------------------------------------------------
// 6. File names: no import-query or virtual-id artifacts on disk
// ---------------------------------------------------------------------------
console.log('\n--- File names ---');
const badNames = globSync('dist/**/*').filter(f => /[?\0]/.test(f));
assert(
  badNames.length === 0,
  `no '?' or NUL bytes in dist file names (got: ${badNames.join(', ') || 'none'})`,
);
assert(
  existsSync('dist/styles/uui-text.css.js'),
  'dist/styles/uui-text.css.js exists (inline CSS module, query stripped)',
);

// ---------------------------------------------------------------------------
// 7. Source consistency: export count matches component folder count
// ---------------------------------------------------------------------------
console.log('\n--- Source consistency ---');
